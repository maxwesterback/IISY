...
def register_ticket(request):
    ...
    try:
        ...
        if entity is None: # if it's a room and not an entity
            ...
            if tickets.count() != 0:
                ...
                canSend = True
            if room.location == "1": # If the room still has default location then open the admin panel to change it
                return redirect('http://newdomain.live/admin/iisy_landing/room/' + str(room.id) + '/change/')

            if(canSend or hours >= room.hoursBetweenTickets): #if there are no previous tickets or if there has been enough time between last and current
            ...
            if entity.location == "1":
                return redirect('http://newdomain.live/admin/iisy_landing/entity/' + str(entity.id) + '/change/')
